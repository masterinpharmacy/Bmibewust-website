...
export async function POST(request) {
  try {
    ...
    const { naam, email, telefoon, leeftijd, geslacht, lengte, gewicht, bmi, bericht } = data;
    ...
    const emailBody = `
      ...
      <p><strong>Lengte:</strong> ${lengte ? `${lengte} cm` : "-"}</p>
      <p><strong>Gewicht:</strong> ${gewicht ? `${gewicht} kg` : "-"}</p>
